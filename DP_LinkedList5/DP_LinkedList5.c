...
int DP_InsertAfterRollNo()
{
    ...
    char iRollNumber[DP_MAX_STRING_LENGTH] = {0};
    ...
    while (pSTempStudent != NULL)
    {
        ...
        if (strcmp(iRollNumber, pSTempStudent->iRollNumber) == 0)
        {
            iReturnValue = DP_CreateStudent(&pSNewStudent);
            if (iReturnValue < DP_SUCCESS)
            {
...
int DP_InsertBeforeRollNo()
{
    ...
    if (strcmp(iRollNumber, pSFirstStudent->iRollNumber) == 0)
    {
        iReturnValue = DP_CreateStudent(&pSNewStudent);
        if (iReturnValue < DP_SUCCESS)
        {
...
int DP_DeleteStudent()
{
    ...
    while ((pSTempStudent != NULL) && (strcmp(iRollNumber, pSTempStudent->iRollNumber) != 0))
    {
        pSPrev = pSTempStudent;
        pSTempStudent = pSTempStudent->pSNext;
    }
...
int DP_SearchStudent()
{
    ...
    char iRollNumber[DP_MAX_ROLLNO_LENGTH] = {0};
...
int DP_DisplaySorted()
{
    ...
    for (pSOuter = pSFirstStudent; pSOuter != NULL; pSOuter = pSOuter->pSNext)
    {
        for (pSInner = pSOuter->pSNext; pSInner != NULL; pSInner = pSInner->pSNext)
        {
            ...
            if (atoi(pSOuter->iRollNumber) > atoi(pSInner->iRollNumber))
            {
                /*Swap student name*/
                ...
                /*Swap roll number*/
                strcpy(arr_cTempRollNumber, pSOuter->iRollNumber);
                strcpy(pSOuter->iRollNumber, pSInner->iRollNumber);
                strcpy(pSInner->iRollNumber, arr_cTempRollNumber);
...
int DP_IsValidRollNumber(int *out_piRollNo)
{
    ...
    if (strlen(out_piRollNo) >= (DP_MAX_ROLLNO_LENGTH - 1))
    {
        return DP_INVALID_ROLLNUMBER;
    }

    while (out_piRollNo[iIterator] != '\0')
    {
...
int DP_IsRollNumberExists(S_STUDENT *in_pSFirstStudent, int in_iRollNumber)
{
    ...
    {
        if (strcmp(pSCurrent->iRollNumber, in_iRollNumber) == 0)
        {
            return DP_SUCCESS;
        }
